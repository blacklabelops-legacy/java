...
def buildImage(dockerWorkspace, imageName, tagName, branchName, args) {
  def branchSuffix = branchName?.trim() ? '-' + branchName : ''
  def image = imageName + ':' + tagName + branchSuffix
  echo 'Building image: ' + image
  sh 'cd ' + dockerWorkspace + ' && docker build --no-cache --build-arg JAVA_DISTRIBUTION=' + args[0] + '--build-arg JAVA_MAJOR_VERSION=' + args[1] + '--build-arg JAVA_UPDATE_VERSION=' + args[2] + '--build-arg JAVA_BUILD_NUMBER=' + args[3] + ' -t ' + image + ' .'
}
